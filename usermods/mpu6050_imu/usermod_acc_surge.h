...
/* This usermod uses accelerometer data to provide a "surge" effect on movement
*/
...
class AccSurge : public Usermod {
  ...
  public:
    ...
    bool readFromConfig(JsonObject& root)
    {
      ...
      configComplete &= getJsonValue(top["sensitivity"], sensitivity, 0.3);
    ...
    void handleOverlayDraw()
    {
      ...
      if (sample_count != last_sample) {        
        ...
        // We use the raw gyro data (angular rate) and "real accel" to compute a tip movement estimate
        auto gyros = Eigen::Map<Eigen::Vector<int16_t,3>>((int16_t*)um_data->u_data[4]);  // 16384 == 2000 deg/s
        //auto acc = Eigen::Map<Eigen::Vector<int16_t,3>>((int16_t*)um_data->u_data[5]);  // 8192 = 1G


        // hope the convention is right
...
const char AccSurge::_name[] PROGMEM = "AccSurge";