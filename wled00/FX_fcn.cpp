...
bool Segment::allocateData(size_t len) {
  ...
  if (data && _dataLen >= len) { // already allocated enough (reduce fragmentation)
    if (call == 0) {
      if (_dataLen < FAIR_DATA_PER_SEG) { // segment data is small
        //DEBUG_PRINTF("--   Clearing data (%d): %p\n", len, this);
  ...
  if (Segment::getUsedSegmentData() + len - _dataLen > MAX_SEGMENT_DATA) {
    // not enough memory
    DEBUG_PRINTF("SegmentData limit reached: %d/%d\n", len, Segment::getUsedSegmentData());
    errorFlag = ERR_NORAM;
    return false;
  }
  #endif
...
void Segment::resetIfRequired() {
  ...
  if (data && _dataLen > 0) {
    ...
    DEBUG_PRINTF("-- Segment %p reset, data cleared\n", this);
  }
...
void Segment::startTransition(uint16_t dur, bool segmentCopy) {
  ...
  if (isInTransition()) {
    if (segmentCopy && !_t->_oldSegment) {
      ...
      if (_t->_oldSegment) {
        ...
        DEBUGFX_PRINTF("-- Updated transition with segment copy: S=%p T(%p) O[%p] OP[%p]\n", this, _t, _t->_oldSegment, _t->_oldSegment->pixels);
  ...
  if (_t) {
    ...
    if (_t->_oldSegment) {
      DEBUGFX_PRINTF("-- Started transition: S=%p T(%p) O[%p] OP[%p]\n", this, _t, _t->_oldSegment, _t->_oldSegment->pixels);
      if (!_t->_oldSegment->isActive()) stopTransition();
    } else {
      DEBUGFX_PRINTF("-- Started transition without old segment: S=%p T(%p)\n", this, _t);
    }
  };
}

void Segment::stopTransition() {
  DEBUG_PRINTF("-- Stopping transition: S=%p T(%p) O[%p]\n", this, _t, _t->_oldSegment);
  delete _t;
  _t = nullptr;
}
...
Segment &Segment::setColor(uint8_t slot, uint32_t c) {
  ...
  //DEBUG_PRINTF("- Starting color transition: %d [0x%X]\n", slot, c);
...
Segment &Segment::setCCT(uint16_t k) {
  ...
  if (cct != k) {
    //DEBUG_PRINTF("- Starting CCT transition: %d\n", k);
...
Segment &Segment::setOpacity(uint8_t o) {
  if (opacity != o) {
    //DEBUG_PRINTF("- Starting opacity transition: %d\n", o);
    startTransition(strip.getTransition(), blendingStyle != TRANSITION_FADE); // start transition prior to change
    opacity = o;
    stateChanged = true; // send UDP/WS broadcast
  }
...
Segment &Segment::setOption(uint8_t n, bool val) {
  ...
  //DEBUG_PRINTF("- Starting option transition: %d\n", n);
...
Segment &Segment::setPalette(uint8_t pal) {
  ...
  if (pal != palette) {
    //DEBUG_PRINTF("- Starting palette transition: %d\n", pal);
...
void WS2812FX::finalizeInit() {
  ...
  DEBUG_PRINTF("Digital buses: %u, I2S buses: %u\n", digitalCount, i2sBusCount);
  ...
  for (auto &bus : busConfigs) {
    ...
    if (mem + I2SdmaMem > MAX_LED_MEMORY + 1024) { // +1k to allow some margin to not drop buses that are allowed in UI (calculation here includes bus overhead)
      DEBUG_PRINTF("Bus %d with %d LEDS memory usage exceeds limit\n", (int)bus.type, bus.count);
  ...
  DEBUG_PRINTF("Estimated buses + pixel-buffers size: %uB\n", mem + I2SdmaMem);
  busConfigs.clear();
  ...
  DEBUG_PRINTF("Heap after buses: %d\n", ESP.getFreeHeap());
  ...
  DEBUG_PRINTF("strip buffer size: %uB\n", getLengthTotal() * sizeof(uint32_t));
  DEBUG_PRINTF("Heap after strip init: %uB\n", getFreeHeapSize());
}
...
void WS2812FX::service() {
  ...
  #ifdef WLED_DEBUG
  if ((_targetFps != FPS_UNLIMITED) && (millis() - nowUp > _frametime)) DEBUG_PRINTF("Slow effects %u/%d.\n", (unsigned)(millis()-nowUp), (int)_frametime);
  #endif
  if (doShow && !_suspend) {
    yield();
  ...
  #ifdef WLED_DEBUG
  if ((_targetFps != FPS_UNLIMITED) && (millis() - nowUp > _frametime)) DEBUG_PRINTF("Slow strip %u/%d.\n", (unsigned)(millis()-nowUp), (int)_frametime);
  #endif
...
void WS2812FX::makeAutoSegments(bool forceReset) {
  ...
  if (autoSegments) { //make one segment per bus
    ...
    DEBUGFX_PRINTF("%d auto segments created.\n", _segments.size());
...
bool WS2812FX::deserializeMap(unsigned n) {
  ...
  if (!readObjectFromFile(fileName, nullptr, pDoc, &filter)) {
    DEBUG_PRINTF("ERROR Invalid ledmap in %s\n", fileName);
    releaseJSONBufferLock();
    return false; // if file does not load properly then exit
  } else
    DEBUG_PRINTF("Reading LED map from %s\n", fileName);
  ...
  if (n == 0 && (!root["width"].isNull() || !root["height"].isNull())) {
    ...
    isMatrix = true;
    DEBUG_PRINTF("LED map width=%d, height=%d\n", Segment::maxWidth, Segment::maxHeight);
  }
  ...
  if (customMappingTable) {
    DEBUG_PRINTF("ledmap allocated: %uB\n", sizeof(uint16_t)*getLengthTotal());
    ...
    for (unsigned i=0; i<customMappingSize; i++) {
      ...
      DEBUG_PRINTF("%4d,", customMappingTable[i] < 0xFFFFU ? customMappingTable[i] : -1);
    }
    DEBUG_PRINTLN();
    #endif
/*
