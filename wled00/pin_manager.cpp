...
bool PinManager::deallocatePin(byte gpio, PinOwner tag)
{
  ...
  if ((ownerTag[gpio] != PinOwner::None) && (ownerTag[gpio] != tag)) {
    WLOG_E("pin", "PIN DEALLOC: FAIL GPIO %d allocated by 0x%02X, but attempted de-allocation by 0x%02X.", gpio, static_cast<int>(ownerTag[gpio]), static_cast<int>(tag));
    return false;
  }
...
bool PinManager::allocateMultiplePins(const managed_pin_type * mptArray, byte arrayElementCount, PinOwner tag)
{
  ...
  for (int i = 0; i < arrayElementCount; i++) {
    ...
    if (!(isPinOk(gpio, mptArray[i].isOutput) || tag==PinOwner::Ethernet)) {
      WLOG_E("pin", "PIN ALLOC: FAIL Invalid pin attempted to be allocated: GPIO %d as %s.", gpio, mptArray[i].isOutput ? "output" : "input");
      shouldFail = true;
    }
    if ((tag==PinOwner::HW_I2C || tag==PinOwner::HW_SPI) && isPinAllocated(gpio, tag)) {
      ...
      WLOG_E("pin", "PIN ALLOC: FAIL GPIO %d already allocated by 0x%02X.", gpio, static_cast<int>(ownerTag[gpio]));
      shouldFail = true;
    }
  }
  if (shouldFail) {
  ...
  for (int i = 0; i < arrayElementCount; i++) {
    ...
    ownerTag[gpio] = tag;
    WLOG_D("pin", "PIN ALLOC: Pin %d allocated by 0x%02X.", gpio, static_cast<int>(tag));
  }
  WLOG_D("pin", "PIN ALLOC: 0x%014llX.", (unsigned long long)pinAlloc);
  return true;
}
...
bool PinManager::allocatePin(byte gpio, bool output, PinOwner tag)
{
  ...
  if (!isPinOk(gpio, output) || (gpio >= WLED_NUM_PINS) || tag==PinOwner::HW_I2C || tag==PinOwner::HW_SPI
    ...
    if (gpio < 255) {  // 255 (-1) is the "not defined GPIO"
      if (!isPinOk(gpio, output)) {
        if (output) WLOG_E("pin", "PIN ALLOC: FAIL for owner 0x%02X: GPIO %d cannot be used for i/o on this MCU.", static_cast<int>(tag), gpio);
        else        WLOG_E("pin", "PIN ALLOC: FAIL for owner 0x%02X: GPIO %d cannot be used as input on this MCU.", static_cast<int>(tag), gpio);
      } else {
        WLOG_E("pin", "PIN ALLOC: FAIL GPIO %d - HW I2C & SPI pins have to be allocated using allocateMultiplePins.", gpio);
      }
    }
    #endif
    return false;
  ...
  if (isPinAllocated(gpio)) {
    WLOG_E("pin", "PIN ALLOC: FAIL Pin %d already allocated by 0x%02X.", gpio, static_cast<int>(ownerTag[gpio]));
    return false;
  }

  bitWrite(pinAlloc, gpio, true);
  ownerTag[gpio] = tag;
  WLOG_D("pin", "PIN ALLOC: Pin %d successfully allocated by 0x%02X.", gpio, static_cast<int>(ownerTag[gpio]));
  WLOG_D("pin", "PIN ALLOC: 0x%014llX.", (unsigned long long)pinAlloc);
